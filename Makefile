...
build-linux-amd64: prepare
	...
	@rm -rf build-tool/crypto-linux-amd64
	@$(MAKE) build \
    		PLATFORM=linux-amd64 \
    		APP_BIN=./bin/$(APP_NAME)-linux-amd64
	@mkdir -p build-tool/crypto-linux-amd64/bin
	@cp bin/crypto-linux-amd64 build-tool/crypto-linux-amd64/bin/crypto
	@cp -r build-tool/default/config build-tool/crypto-linux-amd64/config
	@cp build-tool/default/*.sh build-tool/crypto-linux-amd64/
	@cp build-tool/default/input.txt build-tool/crypto-linux-amd64/
	@cp build-tool/default/README.md build-tool/crypto-linux-amd64/
	zip -r build-tool/crypto-linux-amd64.zip build-tool/crypto-linux-amd64
	@echo "building linux amd64 completed."

.PHONY: build-linux-arm64
build-linux-arm64: prepare
	@echo "building linux arm64 started..."
	@rm -rf build-tool/crypto-linux-arm64
	@$(MAKE) build \
    		PLATFORM=linux-arm64 \
    		APP_BIN=./bin/$(APP_NAME)-linux-arm64
	@mkdir -p build-tool/crypto-linux-arm64/bin
	@cp bin/crypto-linux-arm64 build-tool/crypto-linux-arm64/bin/crypto
	@cp -r build-tool/default/config build-tool/crypto-linux-arm64/config
	@cp build-tool/default/*.sh build-tool/crypto-linux-arm64/
	@cp build-tool/default/input.txt build-tool/crypto-linux-arm64/
	@cp build-tool/default/README.md build-tool/crypto-linux-arm64/
	zip -r build-tool/crypto-linux-arm64.zip build-tool/crypto-linux-arm64
...
clean:
	...
	rm -f ./bin/*
	rm -f ./build-tool/crypto-linux-amd64/*
	rm -f ./build-tool/crypto-linux-arm64/*
